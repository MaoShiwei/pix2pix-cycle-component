...
class Dataset():
    def __init__(self, dataDir='./datasets/facade', data_start = 1, data_end = 100):

        self.dataset = []
        self.shufflelist = []

        for i in range (data_start, data_end + 1):
            real_image = cv2.imread(dataDir+"/contrast/"+i+".png")
            real_image = self.resize(real_image, ipl_alg = cv2.INTER_CUBIC)

            input_x_image = cv2.imread(dataDir+"/origin/"+i+".png")
